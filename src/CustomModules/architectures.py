...
def _logmeanexp(x, axis=0):
    return logsumexp(x, axis=axis) - jnp.log(x.shape[axis])
        ...
        def predictive_model(batch):
            ...
            with numpyro.plate("batch", num_samples):
                ...
                if self.model_mode == "n":    
                    ...
                    numpyro.sample("obs", dist.Bernoulli(logits=x).to_event(1), obs=batch)
...
class GlobalVAE(BaseVAE):
    ...
    def get_guide_m(self):
        def guide_m(**kwargs):
            def a_init(key):
                return dist.Normal(0, 1).expand([self.z_dim]).sample(key)
...
class SteinGlobalVAE(GlobalVAE):
    ...
    def evaluate_iwae(self, dataloader, size, rng_key, num_particles=500):
        ...
        def predictive_qm_dist():
            ...
            mixing = dist.Categorical(probs=jnp.ones(S) / S)

            components = []
            for s in range(S):
                base_s = dist.Normal(a[s], B[s]).to_event(1)
                components.append(base_s)

            return dist.MixtureGeneral(mixing, components)
        ...
        def predictive_model(batch):
            ...
            with numpyro.plate("batch", num_samples):
                ...
                z = numpyro.sample("z", dist.Normal(m, 1).to_event(1))
        ...
        def predictive_guide(batch):
            ...
            with numpyro.plate("batch", batch_dim):
                ...
                broadcasted = m + jnp.zeros((batch_dim, self.z_dim))
                concat_input = jnp.concatenate([batch, broadcasted], axis=1)
